...
const AssignWardCoordinatorModal = ({ handleCloseModal, onCloseModal }) => {
  ...
  const fetchWardsBySubcountyCoordinator = async () => {
    try {
      ...
      // Extract countyIds where subcounty coordinator exists
      const countyIds = hierarchyResponse.data.message
      ...
      // Flatten and filter the valid responses
      const wards = wardsData
        .filter((data) => data !== null)
        .flatMap((county) => county.subcounties.flatMap((sub) => sub.wards));
      setWards(wards);
    } catch (error) {
