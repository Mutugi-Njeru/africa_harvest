...
const CourseModuleModal = ({ isOpen, onClose, onModuleCreated, courseId, courseTitle }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Prepare the request body with durationHours instead of order
    const requestBody = {
    ...
    try {
      ...
      if (response.data.isSuccess) {
        ...
        toast.error(response.data.message || "Module creation failed");
      }
    } catch (error) {
      if (error.response) {
        const errorMessage = error.response.data.message || "Module creation failed";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-white p-6 shadow-lg w-[600px] rounded-lg">
        <h2 className="text-lg font-semibold mb-4">
          Create Module for Course: {courseTitle}
        </h2>
        <form onSubmit={handleSubmit}>
          ...
          {/* Duration Hours - Fixed from order to durationHours */}
          <div className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Duration (Hours)
            </label>
